...
export class CollectionService {
    ...
    public create(name: string, type: CollectionType, parentGroupId: number | null, smartQuery: string): Promise<Collection> {
        switch (type) {
            ...
            case CollectionType.SMART: {
                return this.dbAccess.querySingle(SQL.queryItemByCustomQuery(smartQuery.trim()))
    ...
    public delete(collectionId: number): Promise<void> {
        return this.dbAccess.run(SQL.deleteCollection(collectionId)).then();
    }
        ...
        return this.getById(collectionId)
            ...
            .then((collection: Collection) => {
                switch (collection.type) {
                    case CollectionType.SMART: {
                        return this.dbAccess.querySingle(SQL.queryItemByCustomQuery(newSmartQuery.trim()))
                            .then(() => collection)
    ...
    public move(collectionId: number, targetGroupId: number | null): Promise<void> {
        return this.dbAccess.run(SQL.updateCollectionParent(collectionId, targetGroupId)).then();
    }
