...
describe("import", () => {
	...
	describe("import with errors", () => {
		...
		test("move (file already exists)", async () => {
			...
			expect(fsWrapper.copy).toBeCalledTimes(0);
			expect(statusChannel.sendAndForget).toBeCalledTimes(3);
			expect(importService.isImportRunning()).toBeFalsy();
			await expect(dbAccess.queryAll(SQL.queryItemsAll([]))).resolves.toEqual([
...
function mockItemsImportStatusChannel(): ItemsImportStatusChannel {
	// @ts-ignore
	const channel: ItemsImportStatusChannel = {};
	channel["sendAndForget"] = jest.fn().mockReturnValue(Promise.resolve()) as any;
	channel["send"] = jest.fn().mockReturnValue(Promise.resolve()) as any;
	return channel;
}
