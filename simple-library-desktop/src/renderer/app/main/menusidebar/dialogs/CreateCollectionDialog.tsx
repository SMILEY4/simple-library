...
interface NewCreateCollectionDialogProps {
    parentGroup: Group,
    rootGroup: Group,
    onCancel: () => void,
...
export function CreateCollectionDialog(props: React.PropsWithChildren<NewCreateCollectionDialogProps>): React.ReactElement {
...
function useCollectionType() {
    const [type, setType, refType] = useStateRef(CollectionType.NORMAL);
    return {
        type: type,
...
function useCollectionQuery() {
    const [query, setQuery, refQuery] = useStateRef("");
    return {
        query: query,
